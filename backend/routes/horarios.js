...
function requireAdmin(req, res, next) {
  ...
  if (rol !== 'administrador') {
    return res.status(403).json({ error: 'Solo el administrador puede gestionar horarios.' });
  }
  next();
}
...
router.get('/', async (req, res) => {
  try {
    const [rows] = await pool.query(`
      ...
      ORDER BY p.nombre_completo, FIELD(h.dia_semana,'lunes','martes','miercoles','jueves','viernes','sabado','domingo'), h.hora_inicio
    `);
...
router.post('/', requireAdmin, async (req, res) => {
  ...
  if (!id_psicologo || !dia_semana || !hora_inicio || !hora_fin) {
    return res.status(400).json({ error: 'Psicólogo, día, hora inicio y hora fin son obligatorios.' });
  }
...
router.put('/:id', requireAdmin, async (req, res) => {
  ...
  try {
    await pool.query(`
      ...
      SET id_psicologo = ?, dia_semana = ?, hora_inicio = ?, hora_fin = ?, activo = ?
      WHERE id_horario = ?
    `, [
      id_psicologo,
      dia_semana,
